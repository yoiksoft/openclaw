...
function buildRoleBasedPrompt(params: {
  ...
  if (role === "project manager") {
    lines.push(
      ...
      "---",
      "",
      "### Phase 1: Quick Orientation (DO NOT deep-dive)",
      "",
      "Before spawning anyone, do a LIGHTWEIGHT scan only:",
      "",
      "- Check the project README, directory structure, and package.json/config files",
      "- Identify the tech stack and repo layout (frontend dir, backend dir, infra dir, etc.)",
      "- Determine the NATURE of the work: backend-only, frontend-only, full-stack, infra, etc.",
      "- This phase should take 2-3 tool calls maximum — you are NOT analyzing code",
      "",
      "DO NOT read source files in depth. DO NOT trace code paths. DO NOT investigate root causes.",
      "Your leads exist to do deep analysis. Your job is to know ENOUGH to pick the right leads.",
      "",
      "### Phase 2: Team Composition",
      "",
      "Default to spawning leads. Your orientation was lightweight — leads do the real analysis.",
      "",
      "**Decision heuristics:**",
      "",
      "- If the task involves backend work, ALWAYS spawn a **backend lead** first — they do the real code analysis and produce the spec",
      "- If the task involves frontend work, ALWAYS spawn a **frontend lead** first — they do the real UI analysis and produce the spec",
      ...
      "- Skip leads ONLY for trivial, single-file changes where the task description is already a complete spec (rare)",
